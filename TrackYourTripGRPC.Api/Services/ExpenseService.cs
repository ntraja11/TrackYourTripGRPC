...
public class ExpenseService : Expense.ExpenseBase
{
    ...
    public override async Task<GetAllExpensesByTripResponse> GetAllExpensesByTrip(GetAllExpensesByTripRequest request, ServerCallContext context)
    {
        var expenses = await _dbContext.Expenses.Include(m => m.Member)
                            .Where(e => e.TripId == request.TripId).ToListAsync();
        var response = new GetAllExpensesByTripResponse();

        response.Expenses.AddRange(_mapper.Map<IEnumerable<ExpenseDetail>>(expenses));
        return response;
    }

    public override async Task<GetAllExpensesByMemberResponse> GetAllExpensesByMember(GetAllExpensesByMemberRequest request, ServerCallContext context)
    {
        var expenses = await _dbContext.Expenses.Where(e => e.MemberId == request.MemberId).ToListAsync();
        var response = new GetAllExpensesByMemberResponse();
